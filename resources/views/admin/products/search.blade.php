        ...
        <div class="card card-flush">
            ...
            <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                ...
                <div class="card-title">
                    <!--begin::Search-->
                    <form class="navbar-search form-inline mb-4 w-50 d-flex" action="{{ route('admin.product.search') }}" method="GET">
                        @csrf
                ...
                <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                    <div class="w-100 mw-150px">
                        <!--begin::Select2-->
                        <form id="searchForm" action="{{ route('admin.product.status') }}" method="post">
                            @csrf
                            <!--end::Svg Icon-->
                    ...
                    <tbody class="fw-bold text-gray-600">
                        ...
                        @foreach ($result as $product)
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        ...
                                        <a href="#0" class="symbol symbol-50px">
                                            @if (!empty($product['photo']) && count($product['photo']) > 0)
                                                @php($firstImage = $product['photo'][0])
                                                <span class="symbol-label"
                                                    style="background-image:url({{ asset('site/images/products/' . $firstImage) }});"></span>
                                            @endif
                                        </a>
                                        ...
                                        <div class="ms-5">
                                            ...
                                            <a href="" class="text-gray-800 text-hover-primary fs-5 fw-bolder"
                                                data-kt-ecommerce-product-filter="product_name">{{ $product->title['en'] ?? $product->title['ru'] ?? $product->title['uz'] ?? '' }}</a>
                                            <!--end::Title-->
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="text-end pe-0">
                                    <span class="fw-bolder">{{ $product->product_category->title['en']?? $product->product_category->title['ru']??$product->product_category->title['uz']??'' }}</span>
                                </td>

                                <!--end::SKU=-->
                                <!--begin::Qty=-->
                                <td class="text-end pe-0" data-order="25">
                                    <span class="fw-bolder ms-3">{{ $product->created_at }}</span>
                                </td>
                                ...
                                <td class="text-end pe-0" data-order="Published">
                                    ...
                                    @elseif ($product->status === 'Inacitve')
                                ...
                                <!--begin::Action=-->
                                <td class="text-end">
                                    <form class="" action="{{ route('admin.products.destroy', $product->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
