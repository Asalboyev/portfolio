        ...
        <div class="card card-flush">
            ...
            <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                ...
                <div class="card-title">
                    ...
                    action="{{ route('admin.service.search') }}" method="post">
                    @csrf
                ...
                <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                    <div class="w-100 mw-150px">
                        <!--begin::Select2-->
                        <form id="searchForm"
                        action="{{ route('admin.service.status') }}" method="post">
                      @csrf
                          <!--end::Svg Icon-->
                    ...
                    <!--begin::Add product-->
                    <a href="{{ route('admin.services.create') }}" class="btn btn-primary">Add Servic</a>
                    <!--end::Add product-->
                </div>
                <!--end::Card toolbar-->
            </div>
                        ...
                        @foreach ($services as $servic)
                            <tr>
                                ...
                                <td class="text-end pe-0">
                                    <span class="fw-bolder">{{ $servic->service_category->title['en'] }}</span>
                                </td>
                                <!--end::SKU=-->
                                <!--begin::Qty=-->
                                ...
                                <td class="text-end pe-0" data-order="Published">
                                    @if ($servic->status === 'Active')
                                        <div class="badge badge-light-success">{{ $servic->status }}</div>
                                    @elseif ($servic->status === 'Inacitve')
                                        <div class="badge badge-light-danger">{{ $servic->status }}</div>
                                    @else
                                        <div class="badge badge-light-secondary">{{ $servic->status }}</div>
                                    @endif
                                </td>
                                <!--end::Status=-->
                                <!--begin::Action=-->
                                <td class="text-end">
                                    <form class="" action="{{ route('admin.services.destroy', $servic->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
